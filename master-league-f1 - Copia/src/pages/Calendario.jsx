...
function Calendario() {
    ...
    const DriverImage = ({ name, season, style }) => {
        ...
        const [imgSrc, setImgSrc] = useState(`/pilotos/${gridType}/s${season}/${cleanName}.png`);
        
        useEffect(() => {
            setImgSrc(`/pilotos/${gridType}/s${season}/${cleanName}.png`);
        }, [gridType, season, cleanName]);
        
        const handleError = () => {
            if (imgSrc !== '/pilotos/pilotoshadow.png') setImgSrc('/pilotos/pilotoshadow.png');
        };
    ...
    return (
        <div style={{minHeight: '100vh', background: 'var(--bg-dark-main)', color: 'white', padding: '80px 20px 40px', fontFamily: "'Montserrat', sans-serif"}}>
            <div style={{maxWidth: '1400px', margin: '0 auto'}}>
                    ...
                    <div style={{display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap'}}>
                        <div style={{display: 'flex', gap: '10px'}}>
                            <button 
                                ...
                                onMouseEnter={(e) => e.target.style.background = gridType === 'carreira' ? 'var(--carreira-wine)' : 'rgba(255,255,255,0.2)'}
                                onMouseLeave={(e) => e.target.style.background = gridType === 'carreira' ? 'var(--carreira-wine)' : 'rgba(255,255,255,0.1)'}
                            >
                                GRID CARREIRA
                            </button>
                            <button 
                                ...
                                onMouseEnter={(e) => e.target.style.background = gridType === 'light' ? 'var(--light-blue)' : 'rgba(255,255,255,0.2)'}
                                onMouseLeave={(e) => e.target.style.background = gridType === 'light' ? 'var(--light-blue)' : 'rgba(255,255,255,0.1)'}
                            >
                                GRID LIGHT
                            </button>
                        </div>
                ...
                {races.length > 0 ? (
                    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(500px, 1fr))', gap: '24px', paddingBottom: '40px'}}>
                        {races.map((race, idx) => {
                            return (
                                <div 
                                    key={idx}
                                ...
                                >
                                    ...
                                    <div style={{flex: 1.5, padding: '28px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden'}}>
                                        ...
                                        <div style={{position: 'relative', zIndex: 2}}>
                                            ...
                                            <div style={{display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '18px'}}>
                                                ...
                                                <p style={{fontSize: '0.85rem', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '2px', margin: 0, fontWeight: '800'}}>
                                                    ROUND {race.round}
                                                </p>
                                            </div>

                                            {/* Nome do GP */}
                                            <h3 style={{fontSize: '2rem', fontWeight: '900', margin: '0 0 12px 0', color: 'white', lineHeight: '1.1'}}>
                                                {race.gp}
                                            </h3>

                                            {/* Nome do Circuito */}
                                            {race.circuitName && (
                                                <p style={{fontSize: '1rem', color: '#06B6D4', margin: '0 0 16px 0', fontWeight: '700'}}>
                                                    🏁 {race.circuitName}
                                                </p>
                                            )}

                                            {/* Data */}
                                            <p style={{fontSize: '1.1rem', fontWeight: '700', margin: 0, color: '#94A3B8'}}>

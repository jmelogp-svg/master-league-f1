...
function Telemetria() {
    ...
    const { evolutionData, qualyData, racePaceData, topDriversList } = useMemo(() => {
        ...
        data.forEach(row => {
            ...
            if (!isNaN(qualy) && !isNaN(race)) {
                driverMap[name].qualySum += qualy;
                driverMap[name].raceSum += race;
                driverMap[name].deltaSum += (qualy - race); 
                driverMap[name].racesCount++;
            }

            driverMap[name].totalPoints += points;
            driverMap[name].pointsHistory[round] = driverMap[name].totalPoints;
        });

        const sortedDrivers = Object.values(driverMap).sort((a, b) => b.totalPoints - a.totalPoints);
        ...
        const evolData = rounds.map(r => {
            ...
            topDrivers.forEach(d => {
                let pts = 0;
                for(let i = r; i >= 1; i--) { if (d.pointsHistory[i] !== undefined) { pts = d.pointsHistory[i]; break; } }
                point[d.name] = pts;
            });
            return point;
        });
    ...
    }, [gridType, selectedSeason, rawCarreira, rawLight]);
                    ...
                    <>
                                ...
                                <ResponsiveContainer>
                                    <LineChart data={evolutionData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                                        ...
                                        {topDriversList.map((driver) => {
                                            const isSecondDriver = usedTeams.has(driver.team);
                                            usedTeams.add(driver.team);
                                            return (
                                                <Line 
                        ...
                        <div className="chart-grid-row" style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px'}}>
                            ...
                            <div className="chart-card" style={{width: '100%'}}>
                                ...
                                <div style={{ width: '100%', height: 600 }}>
                                    <ResponsiveContainer>
                                        <BarChart data={racePaceData} layout="vertical" margin={{top: 5, right: 50, left: 40, bottom: 5}}>
                                            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} />
                                            ...
                                            <Bar dataKey="delta" name="Delta" barSize={15}>
                                                <LabelList dataKey="delta" position="right" fill="white" fontSize={11} fontWeight={800} formatter={(val) => (val > 0 ? `+${val}` : val)} />

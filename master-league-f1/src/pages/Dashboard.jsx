...
const DriverImage = ({ name, gridType, season }) => {
    ...
    const s = season || '19';
    const primarySrc = `/pilotos/${gridType || 'carreira'}/s${s}/${cleanName}.png`;
    ...
    return <img src={primarySrc} onError={handleError} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="" />;
};
...
function Dashboard() {
    ...
    const [acusacoesPendentes, setAcusacoesPendentes] = useState(0); // Badge de notificações
    ...
    // Buscar acusações pendentes contra o piloto logado (sem defesa incorporada)
    useEffect(() => {
        const buscarAcusacoesPendentes = async () => {
            if (!profile?.nome_piloto) return;
            ...
            try {
                ...
                // Filtrar acusações contra este piloto e que NÃO têm defesa INCORPORADA
                const acusacoesSemDefesa = (data || []).filter(notif => {
                    const dados = notif.dados || {};
                    const acusado = dados.acusado || {};
                    const temDefesa = dados.defesa != null; // Defesa incorporada no próprio registro
                    return acusado.nome?.toUpperCase() === profile.nome_piloto?.toUpperCase() && !temDefesa;
                });

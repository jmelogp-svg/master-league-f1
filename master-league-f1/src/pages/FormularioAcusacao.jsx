...
Aguarde análise dos Stewards.`
    ...
    // Gerar URL da foto do piloto
    // Nota: Usando s19 pois ainda não há pasta s20 com as fotos
    ...
    const getSmlFotoUrl = (piloto) => {
        if (!piloto) return '/pilotos/pilotoshadow.png';
        return `/pilotos/SML/${piloto.fotoNome}.png`;
    };
    
    const handleFotoError = (e, piloto) => {
        if (e.target.src.includes(`/s${TEMPORADA_FOTOS}/`)) {
            e.target.src = getSmlFotoUrl(piloto);
